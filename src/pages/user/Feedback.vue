  ...
  >
    <VaCard class="bg-white rounded-3xl shadow-2xl p-8 max-w-lg w-full relative">
      ...
      <div class="text-center mb-8">
        <h2 class="text-3xl font-bold mb-3">{{ t('vuestic.feedback_mess') }}</h2>
        <p class="">{{ t('vuestic.feedback_type') }}</p>
      </div>
      ,
      <VaForm class="space-y-6" @submit.prevent="submitFeedback">
        ...
        <p v-if="rating === 0" class="text-sm text-red-500 text-center">{{ t('vuestic.feedback_rating') }}</p>
        ...
        <div>
          ...
          ></VaTextarea>
          <p v-if="!comment" class="text-sm text-red-500 text-center">{{ t('vuestic.feedback_placeholder_error') }}</p>
        </div>
        ...
        >
          <span v-if="isSubmitting" class="animate-pulse">{{ t('vuestic.feedback_submiting') }}</span>
          <span v-else>{{ t('vuestic.feedback_submit') }}</span>
        </button>
      </VaForm>
    </VaCard>
