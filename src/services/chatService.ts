...
class ChatService {
  ...
  public async sendMessage(receiverId: string, message: string, isStaffSender: boolean) {
    try {
      await signalRService.invoke(this.hubName, 'SendMessage', receiverId, message, isStaffSender)
    } catch (err) {
  ...
  public async joinPatientGroup(patientId: string) {
    try {
      await signalRService.invoke(this.hubName, 'JoinPatientGroup', patientId)
    } catch (err) {
      console.error('Error joining patient group:', err)
    }
  }

  public async getConversation(patientId: string) {
    try {
      await signalRService.invoke(this.hubName, 'GetConversation', patientId)
  ...
  public async leavePatientGroup(patientId: string) {
    try {
      await signalRService.invoke(this.hubName, 'LeavePatientGroup', patientId)
    } catch (err) {
